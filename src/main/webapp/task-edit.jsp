...
<body>
				...
				<div class="row">
					...
					<div class="col-md-8 col-xs-12">
						<div class="white-box">
							...
							<form class="form-horizontal form-material" action="edit_task?typee=2" method="post">
    <div class="form-group">
        <label class="col-md-12">ID Task</label>
        <div class="col-md-12">
            <input type="text" name="idT" class="form-control form-control-line" value="${task.id}" readonly>
        </div>
    </div>
    <div class="form-group">
        <label class="col-md-12">Dự án</label>
        <div class="col-md-12">
            <select class="form-control form-control-line" name="proj">
                <c:forEach var="item" items="${ListProjects}">
                    <option value="${item.getId()}" ${item.getId() == task.projectEntity.id ? 'selected="selected"' : ''}>${item.getName()}</option>
                </c:forEach>
            </select>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-md-12">
            <input type="text" name="name" placeholder="Tên công việc" value="${task.name}" class="form-control form-control-line">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-md-12">
            <select class="form-control form-control-line" name="user">
                <c:forEach var="item" items="${ListUsers}">
                    <option value="${item.getId()}" >${item.getlName()}</option>
                </c:forEach>
            </select>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-md-12">
            <input type="text" name="startDate" placeholder="dd/MM/yyyy" value="${task.start_date}" class="form-control form-control-line">
        </div>
    </div>
    <div class="form-group">
        <label class="col-md-12">Ngày kết thúc</label>
        <div class="col-md-12">
            <input type="text" name="endDate" placeholder="dd/MM/yyyy" value="${task.end_date}" class="form-control form-control-line">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-md-12">
            <select class="form-control form-control-line" name="status">
                <c:forEach var="item" items="${ListStatus}">
                    <option value="${item.getId()}" ${item.getId() == task.statusEntity.id ? 'selected="selected"' : ''}>${item.getName()}</option>
                </c:forEach>
            </select>
        </div>
    </div>
